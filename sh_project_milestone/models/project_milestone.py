...
class ProjectTask(models.Model):
    ...
    def create(self,vals):
        if vals.get('project_id'):
            if 'milestone_ids' in vals:
                milestone_ids = vals.get('milestone_ids')[0][2]
                project = self.env['project.project'].sudo().browse(vals.get('project_id'))
                if project and project.milestone_ids:
                    p_milestone_ids = project.milestone_ids.ids
                    milestone_to_add = [x for x in milestone_ids if x in p_milestone_ids]
                    vals.update({
                        'milestone_ids':[(6,0,milestone_to_add)]
                    })
