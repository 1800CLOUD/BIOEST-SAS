...
class MrpReport2(models.Model):
    _name = 'mrp.report.2'
    _description = 'Reporte Manufactura 2'
    ...
    wc_account_id = fields.Many2one('account.account', 'Cuenta centro de producción', readonly=True)
    ...
    def _select(self):
        select_str = """
            SELECT
                ...
                op_cost.wc_id		   AS workcenter_id,
                op_cost.wc_account_id  AS wc_account_id,
                ...
                comp_cost.total/(CASE WHEN wc_count.wc_count > 0 THEN wc_count.wc_count ELSE 1 END) * currency_table.rate                                                                                   AS component_cost,
                op_cost.total * currency_table.rate                                                                                     AS operation_cost,
                (comp_cost.total/(CASE WHEN wc_count.wc_count > 0 THEN wc_count.wc_count ELSE 1 END) + op_cost.total) * currency_table.rate                                                                 AS total_cost,
        ...
        from_str = """
            ...
            LEFT JOIN (
                SELECT
                    mo_id                                                                    AS mo_id,
                    wc_id																	 AS wc_id,
                    wc_account_id															 AS wc_account_id,
                ...
                FROM (
                    SELECT
                        mo.id AS mo_id,
                        wc.id AS wc_id,
                        substr(ip.value_reference, 17)::integer AS wc_account_id,
                        CASE
                    ...
                    LEFT JOIN ir_property ip ON ip.name = 'account_expense_id'
                                            AND ip.res_id = 'mrp.workcenter,' || wc.id
                                            AND ip.company_id = mo.company_id
                    WHERE mo.state = 'done'
                    GROUP BY
                        mo.id,
                        wc.id,
                ...
                GROUP BY mo_id, wc_id, wc_account_id
            ...
            LEFT JOIN (
                ...
                COUNT(DISTINCT wc.id) AS wc_count
                FROM mrp_production  AS mo
                LEFT JOIN mrp_workorder AS wo ON wo.production_id = mo.id
                LEFT JOIN mrp_workcenter AS wc ON wc.id = wo.workcenter_id
                GROUP BY mo.id
            ) wc_count ON wc_count.mo_id = mo.id
        """.format(
